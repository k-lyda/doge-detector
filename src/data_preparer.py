...
class DataPreparer():
    def crop_images(self, images_path, image_size, output_folder='dataset/Cropped'):
        ...
        for entry in tqdm_notebook(os.scandir(images_path), total=total_folders, unit="folders"):
            if not entry.name.startswith('.') and entry.is_dir():
                self.crop_folder_images(entry.path, image_size)

    def crop_folder_images(self, folder_path, image_size, output_folder='dataset/Cropped'):
        for entry in os.scandir(folder_path):
            if not entry.name.startswith('.') and entry.is_file():
                self.crop_image(entry.path, image_size)

    def crop_image(self, image_path, image_size, output_folder='dataset/Cropped'):
        try:
